    ...
    if isempty(yi_arr) == false
        ...
        for ix = 1:length(yi_arr),
            ...
            for h = 1:maxhw,
                ...
                %{
                if (ix == 18359)
                    disp(['interres ', num2str(yixL), '/', num2str(yixU), '/', num2str(xixL), '/', num2str(xixU)]);
                    disp(map(yixL:yixU,xixL:xixU))
                end
                %}
            ...
            if nz == true,
                ...
                if (cnt == 0)
                    ...
                    win
                    disp('mapr');
                    map(yixL:yixU, xixL:xixU)
                    disp('i/j');
                    find(map(yixL:yixU, xixL:xixU)~=0)
                end
                cnt = cnt + 1;
            end
